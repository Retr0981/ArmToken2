...
contract(ArmahToken, accounts => {
  let tokenInstace;
  it("initializes with the correct values", () => {
    return ArmahToken.deployed()
      .then(instance => {
        tokenInstace = instance;
        return tokenInstace.name();
      })
      .then(name => {
        assert.equal(name, "Armah Token", "has the correct name");
        return tokenInstace.symbol();
      })
      .then(symbol => {
        assert.equal(symbol, "ARM", "has the correct symbol");
        return tokenInstace.standard();
      })
      .then(standard => {
        assert.equal(standard, "ArmahToken v1.0", "has the correct standard");
      });
  });
  ...
  it("approves delegatd transfer", () => {
    return ArmahToken.deployed().then(instance => {
      tokenInstace = instance;
